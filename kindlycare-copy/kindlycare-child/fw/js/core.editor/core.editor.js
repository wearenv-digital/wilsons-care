jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').on('click', function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(KINDLYCARE_STORAGE['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').on('click', function (e) {
		"use strict";
		if (jQuery(this).val() == KINDLYCARE_STORAGE['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').on('click', function (e) {
		...
		jQuery.post(KINDLYCARE_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				kindlycare_message_success('', KINDLYCARE_STORAGE['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(KINDLYCARE_STORAGE['strings']['editor_caption_close']);
			} else {
				kindlycare_message_warning(rez.error, KINDLYCARE_STORAGE['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').on('click', function (e) {
		"use strict";
		kindlycare_message_confirm(KINDLYCARE_STORAGE['strings']['editor_delete_post'], KINDLYCARE_STORAGE['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(KINDLYCARE_STORAGE['ajax_url'], data, function(response) {
				...
				if (rez.error == '') {
					kindlycare_message_success('', KINDLYCARE_STORAGE['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = KINDLYCARE_STORAGE['site_url'];
						}, 1000);
				} else {
					kindlycare_message_warning(rez.error, KINDLYCARE_STORAGE['strings']['editor_delete_error']);
				}
			});
