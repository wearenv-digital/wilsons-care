...
function kindlycare_init_actions() {
	...
	// Hide preloader
//	jQuery('body.preloader>.outer_wrap,body.preloader>.body_wrap,body.preloader>.kindlycare_profiler').animate({opacity:1}, 'slow');	//.css({opacity:1});
		...
		if (scroll_offset + jQuery(window).height() > KINDLYCARE_STORAGE['side_panel_height'] + 100) {
			if (jQuery('.sidebar_outer').css('position')!=='fixed') {
				jQuery('.sidebar_outer').css({
					//'top': (scroll_offset + jQuery(window).height() - KINDLYCARE_STORAGE['side_panel_height'] - 100) + 'px'
					'position': 'fixed',
...
function kindlycare_init_post_formats() {
	...
	} else if (KINDLYCARE_STORAGE['popup_engine']=='magnific') {
