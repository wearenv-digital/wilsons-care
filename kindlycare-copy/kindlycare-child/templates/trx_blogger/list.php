...
if ( !function_exists( 'kindlycare_template_list_theme_setup' ) ) {
	add_action( 'kindlycare_action_before_init_theme', 'kindlycare_template_list_theme_setup', 1 );
	function kindlycare_template_list_theme_setup() {
		kindlycare_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'kindlycare')
			));
	}
}

// Template output
if ( !function_exists( 'kindlycare_template_list_output' ) ) {
	function kindlycare_template_list_output($post_options, $post_data) {
