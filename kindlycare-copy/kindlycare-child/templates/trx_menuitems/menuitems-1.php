...
if ( !function_exists( 'kindlycare_template_menuitems_1_theme_setup' ) ) {
	add_action( 'kindlycare_action_before_init_theme', 'kindlycare_template_menuitems_1_theme_setup', 1 );
	function kindlycare_template_menuitems_1_theme_setup() {
		kindlycare_add_template(array(
			'layout' => 'menuitems-1',
			'template' => 'menuitems-1',
			'mode'   => 'menuitems',
			'title'  => esc_html__('MenuItems /Style 1/', 'kindlycare'),
			'thumb_title'  => esc_html__('Avatar (small)', 'kindlycare'),
			'w'		 => 75,
			'h'		 => 75
		));
	}
...
// Template output
if ( !function_exists( 'kindlycare_template_menuitems_1_output' ) ) {
	function kindlycare_template_menuitems_1_output($post_options, $post_data) {
			...
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?> 
				...
				if ($show_title) {
					...
					if ($post_options['popup'] == 'yes')
						...
						echo '<a href="'.esc_url($post_data['post_link']).'">';
					kindlycare_show_layout($post_data['post_title']) . '</a>';
					?></h5><?php 
				}
				?>
				
				<div class="sc_menuitem_description"><?php kindlycare_show_layout(kindlycare_strshort($post_data['post_excerpt'], isset($post_options['descr']) ? $post_options['descr'] : kindlycare_get_custom_option('post_excerpt_maxlength_masonry'))) ?></div>
