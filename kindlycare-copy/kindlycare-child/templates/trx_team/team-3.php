...
if ( !function_exists( 'kindlycare_template_team_3_theme_setup' ) ) {
	add_action( 'kindlycare_action_before_init_theme', 'kindlycare_template_team_3_theme_setup', 1 );
	function kindlycare_template_team_3_theme_setup() {
		kindlycare_add_template(array(
			'layout' => 'team-3',
			'template' => 'team-3',
			'mode'   => 'team',
			/*'container_classes' => 'sc_slider_nopagination sc_slider_controls sc_slider_controls_side',*/
			'title'  => esc_html__('Team /Style 3/', 'kindlycare'),
...
// Template output
if ( !function_exists( 'kindlycare_template_team_3_output' ) ) {
	function kindlycare_template_team_3_output($post_options, $post_data) {
		$show_title = true;
        $team_title = kindlycare_get_post_title($post_data['post_id']);
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				<div class="sc_team_item_avatar"><?php kindlycare_show_layout($post_options['photo']); ?>
					<div class="sc_team_item_hover">
						<div class="sc_team_item_socials"><?php kindlycare_show_layout($post_options['socials']); ?></div>
					</div>
				</div>
				<div class="sc_team_item_info">
					<h5 class="sc_team_item_title"><?php echo (!empty($post_options['link']) ? '<a href="'.esc_url($post_options['link']).'">' : '') . ($team_title) . (!empty($post_options['link']) ? '</a>' : ''); ?></h5>
