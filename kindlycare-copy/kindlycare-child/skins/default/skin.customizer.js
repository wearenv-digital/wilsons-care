...
function kindlycare_skin_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = KINDLYCARE_STORAGE['co_add_params']!=undefined ? KINDLYCARE_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#kindlycare-customizer-styles-'+option).length > 0 ? jQuery('#kindlycare-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="kindlycare-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
